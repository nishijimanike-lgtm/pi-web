...
const { expandInlineSkills, extractSlashQuery } = await jiti.import("./skill-inline.ts");
...
test("extractSlashQuery detects slash and backslash at start or after whitespace", () => {
  // At start
  assert.deepEqual(extractSlashQuery("/"), { start: 0, query: "", prefixChar: "/" });
  assert.deepEqual(extractSlashQuery("/doc"), { start: 0, query: "doc", prefixChar: "/" });
  assert.deepEqual(extractSlashQuery("\\"), { start: 0, query: "", prefixChar: "\\" });
  assert.deepEqual(extractSlashQuery("\\doc"), { start: 0, query: "doc", prefixChar: "\\" });

  // Inline after whitespace
  assert.deepEqual(extractSlashQuery("hello \\doc"), { start: 6, query: "doc", prefixChar: "\\" });
  assert.deepEqual(extractSlashQuery("请用 /pdf"), { start: 3, query: "pdf", prefixChar: "/" });
...
test("expandInlineSkills expands multiple skills anywhere in prompt text", () => {
  ...
  assert.ok(result.includes('<skill name="docx" location="/Users/test/.agents/skills/docx/SKILL.md">'));
  assert.ok(result.includes('<skill name="pdf" location="/Users/test/.agents/skills/pdf/SKILL.md">'));
